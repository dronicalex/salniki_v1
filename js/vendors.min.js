...
window.addEventListener('scroll', function () {
	...
	if (scrollY > 50) {
		headerContainer.classList.add('_scrolling')
	} else {
		headerContainer.classList.remove('_scrolling')
	}
})


let burger = document.querySelector('.header__burger')
let burgerUl = document.querySelector('.menu-header__ul')
let body = document.querySelector('body')
let logo = document.querySelector('.header__logo')
burger.addEventListener('click', function () {
	burger.classList.toggle('_active')
	burgerUl.classList.toggle('_active')
...
// select------------------------------
let selectCar = document.querySelector('.select-car')
...
for (const key in cars) {
	if (Object.hasOwnProperty.call(cars, key)) {
		...
		selectCar.insertAdjacentHTML('beforeend', `<option value="${key}">${key}</option>`)
	}
}

selectCar.addEventListener("change", function (event) {
	selectModel.innerHTML = ''
	selectModel.insertAdjacentHTML('beforeend', `<option value="Не выбрано">Не выбрано</option>`)
			...
			if (key == val) {
				for (const key in element) {
					if (Object.hasOwnProperty.call(element, key)) {
						...
						selectModel.insertAdjacentHTML('beforeend', `<option value="${el}">${el}</option>`)
					}
				}
...
for (let index = 1995; index <= new Date().getFullYear(); index++) {
	yars.push(index)
	selectYear.insertAdjacentHTML('beforeend', `<option value="${index}">${index}</option>`)
}
// --------------------------
// _loading
let infoPriceBlock = document.querySelector('.info-price__container')
let infoPriceBtn = document.querySelector('.info-price__btn')
let priceBlick = document.querySelector('.price')
let priceBlickHeight = priceBlick.clientHeight

priceBlick.style.height = 0
infoPriceBtn.addEventListener('click', function (e) {

	infoPriceBlock.classList.add('_loading')

	setTimeout(() => {
		infoPriceBlock.classList.remove('_loading')
		priceBlick.style.height = `${priceBlickHeight}px`
		calk()
		calkSale()
	...
	setTimeout(() => {
		let priceBlickTarget = priceBlick.getBoundingClientRect().top;
		window.scrollBy({
			top: priceBlickTarget - 30,
			behavior: 'smooth'
...
let center = [47.780915, 27.926181];
...
let typeGid = document.querySelector('#gidravlic')
let typeMeck = document.querySelector('#mechanik')
let typeElec = document.querySelector('#electrik')
let typeNou = document.querySelector('#nou')
let sniataYes = document.querySelector('#yes')
let sniataNo = document.querySelector('#no')
let problemaStuck = document.querySelector('#stuck')
let problemaTech = document.querySelector('#techiet')
let problemaHrust = document.querySelector('#hrust')
let problemaGidravlica = document.querySelector('#gidravlica')
let problemaRuli = document.querySelector('#ruli')

let summ = document.querySelector('#pricee')
let time = document.querySelector('#timee')
let moreText = document.querySelector('._more-text')
...
function calk() {
	...
	if (typeGid.checked) {
		if (problemaStuck.checked) {
			sumProblema = 460
		}
		if (problemaTech.checked) {
			sumProblema = 650
		}
		if (problemaHrust.checked) {
			sumProblema = 400
		}
		if (problemaHrust.checked && problemaTech.checked) {
			sumProblema = 1050
		}
		if (problemaHrust.checked && problemaStuck.checked) {
			sumProblema = 860
		}
		if (problemaTech.checked && problemaStuck.checked) {
			sumProblema = 1110
		}
		if (problemaTech.checked && problemaStuck.checked && problemaHrust.checked) {
			sumProblema = 1510
		}
		if (problemaGidravlica.checked) {
			moreText.innerHTML = 'Если у вас пропадает гидравлика, скорее всего, проблема в насосе ГУР и понадобится замена системы. Пожалуйста, обратитесь к специалисту за более подробной консультацией: 060 777 016'
		}
	}
	if (typeMeck.checked) {
		if (problemaStuck.checked || problemaTech.checked || problemaHrust.checked || problemaGidravlica.checked || problemaRuli.checked) {
			sumProblema = 1280
			moreText.innerHTML = 'Стоимость зависит от того, насколько серьёзные повреждения в вашей рейке. В случае, если окажется, что она неремонтопригодна, рейки есть в наличии в нашем автомагазине'
		}
	}
	if (typeElec.checked) {
		if (problemaStuck.checked) {
			sumProblema = 1045
			moreText.innerHTML = 'Электрическая рулевая рейка - очень сложный узел, причины у стука могут быть самыми разными, и цена меняется в соответствии с ними. Также в наличии есть рейки для вашего автомобиля. Проконсультируйтесь, пожалуйста, со специалистом: 060 777 016'
		}
		if (problemaHrust.checked) {
			sumProblema = 2090
			moreText.innerHTML = 'Электрическая рулевая рейка - очень сложный узел, причины у стука могут быть самыми разными, и цена меняется в соответствии с ними. Также в наличии есть рейки для вашего автомобиля. Проконсультируйтесь, пожалуйста, со специалистом: 060 777 016'
		}
		if (problemaRuli.checked) {
			sumSniata = ''
			sumType = ''
			sumProblema = '--'
			moreText.innerHTML = 'В вашем случае ремонт будет слишком дорогостоящим, поэтому мы обычно предлагаем сразу посмотреть по наличию варианты рулевых реек для вашего автомобиля. Проконсультируйтесь, пожалуйста, со специалистом: 060 777 016'
		}

	}

	totalSum = sumSniata + sumType + sumProblema
	if (typeNou.checked) {
		if (sniataYes.checked) {
			totalSum = 3000
		}
		if (sniataNo.checked) {
			totalSum = 2000
		}
		moreText.innerHTML = 'Это средняя стоимость ремонта рейки. Для более точного расчёта, проконсультируйтесь, пожалуйста со специалистом по номеру 060777016, или оставьте заявку, и мы с вами свяжемся'

	}
	summ.innerHTML = totalSum;


	let inputsArr = document.querySelectorAll('.reica__input_check input')

	let i = 0;
	for (let index = 0; index < inputsArr.length; index++) {

		const element = inputsArr[index];
		if (element.checked) {
			i++

		}
	}
	if (typeMeck.checked || typeGid.checked) {
		if (i == 1) {
			time.innerHTML = 8
		}
		if (i >= 2) {
	...
	if (typeElec.checked) {
		time.innerHTML = '2-5'
		let moreTimeText = document.querySelector('._more-time')
		moreTimeText.innerHTML = 'Время ремонта может отличаться в зависимости от наличия блока рейки и времени дефектовки'
	}
	if (typeNou.checked) {
		time.innerHTML = '7'
	}
...
function calkSale() {
	document.querySelector('._oldd').innerHTML = totalSum + 'MDL'
	document.querySelector('.sale-price-block-promotion__price div').innerHTML = Math.round(totalSum - (totalSum / 100 * 15)) + 'MDL'
	document.querySelector('._infoo').style.display = 'none'
}
...
const formTree = document.getElementById('form-tree')
...
async function formSend(e) {
	...
	if (vall()) {
		...
		let typeContact = document.querySelectorAll('.request__tel input')
		let ttype
		for (let index = 0; index < typeContact.length; index++) {
			const element = typeContact[index];
			if (element.checked) {
				ttype = element.id
			}

		}
		let requestSale = document.querySelectorAll('.request__sale input')
		let requestSaleValue;
		for (let index = 0; index < requestSale.length; index++) {
			const element = requestSale[index];
			if (element.checked) {
				requestSaleValue = element.id
			}
		}


		let formDataOne = new FormData(formOne)
		let formDataTwo = new FormData(formTwo)
		let formDataThree = new FormData(formTree)
		let name = document.querySelector('#name').value
		let sity = document.querySelector('#sity').value
		let tel = document.querySelector('#tel').value

		let reicaCheck;
		let reica = document.querySelectorAll('.reica__tips input')
		let reicaLabel = document.querySelectorAll('.reica__tips label');
		let sniataCheck;
		let sniata = document.querySelectorAll('.reica__take input')
		let sniataLabel = document.querySelectorAll('.reica__take label');
		let problemaCheck = [];
		let problema = document.querySelectorAll('.reica__problems input')
		let problemaLabel = document.querySelectorAll('.reica__problems label');
		for (let index = 0; index < reica.length; index++) {
			const element = reica[index];
			if (element.checked) {
				reicaCheck = reicaLabel[index].textContent
			}
		}
		for (let index = 0; index < sniata.length; index++) {
			const element = sniata[index];
			if (element.checked) {
				sniataCheck = sniataLabel[index].textContent
			}
		}
		for (let index = 0; index < problema.length; index++) {
			const element = problema[index];
			if (element.checked) {
				problemaCheck.push(problemaLabel[index].textContent)
			}
		}

		formDataTwo.append('car', selectCar.value)
		formDataTwo.append('model', selectModel.value)
		formDataTwo.append('year', selectYear.value)
		formDataTwo.append('name', name)
		formDataTwo.append('sity', sity)
		formDataTwo.append('tel', tel)
		formDataTwo.append('typeComunication', ttype)
		formDataTwo.append('sale', requestSaleValue)
		formDataTwo.append('totalSum', totalSum)
		formDataTwo.append('totalTime', time.textContent)
		formDataTwo.append('reica', reicaCheck)
		formDataTwo.append('sniataCheck', sniataCheck)
		formDataTwo.append('problemaCheck', problemaCheck.toString())
		// formDataTwo.append('reica', reicaCheck)
